---
category: skill
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: +2 Gesellschaft/Diplomatie
examples: []
id: feat.general.netzwerker
legacy:
  notes: []
  paths:
    - feats/feats_overview.md
    - feats/feat_netzwerker.md
level: 1
name: Netzwerker
prerequisites:
  - all:
      - attribute:
          gte: 13
          id: charisma
references: []
rulesText: "# Netzwerker\r

  \r

  **Quelle:** Welt-Regelwerk (Zeitalter des Goldes)\r

  \r

  ---\r

  \r

  ## **Flavortext**\r

  \r

  > „Es ist nicht was du weißt, sondern wen du kennst.\"\r

  \r

  Netzwerker ist eine Fähigkeit, die es Charakteren ermöglicht, ihre sozialen Verbindungen und Kontakte optimal zu nutzen. In der urbanen Welt des Zeitalters des Goldes, wo Beziehungen und Netzwerke oft wichtiger sind als individuelle Fähigkeiten, ist diese Fähigkeit von unschätzbarem Wert.\r

  \r

  ---\r

  \r

  ## **Übersicht**\r

  \r

  Netzwerker verbessert die Fähigkeit eines Charakters, in urbaner Umgebung zu navigieren und seine sozialen Kontakte zu nutzen. Diese Fähigkeit ist besonders nützlich für Diplomaten, Händler und alle, die auf soziale Interaktionen angewiesen sind.\r

  \r

  ---\r

  \r

  ## **Mechanische Details**\r

  \r

  ### **Voraussetzungen**\r

  - **Stufe:** 1+\r

  - **Charisma:** 13+\r

  \r

  ### **Effekt**\r

  - **Gesellschaft:** +2 auf Gesellschaftswürfe in urbaner Umgebung\r

  - **Diplomatie:** +2 auf Diplomatiewürfe in urbaner Umgebung\r

  - **Stapelbar:** Nein (nur einmal erhältlich)\r

  \r

  ### **Anwendungsbereiche**\r

  - **Netzwerken:** Neue Kontakte knüpfen\r

  - **Informationen:** Informationen über Kontakte sammeln\r

  - **Einfluss:** Einfluss über Kontakte ausüben\r

  - **Zugang:** Zugang zu exklusiven Bereichen erhalten\r

  \r

  ---\r

  \r

  ## **Spielmechanische Auswirkungen**\r

  \r

  ### **Vorteile**\r

  - **Soziale Kontrolle:** Bessere Kontrolle über soziale Situationen\r

  - **Informationen:** Zugang zu wichtigen Informationen\r

  - **Einfluss:** Einfluss über wichtige Personen ausüben\r

  - **Zugang:** Zugang zu exklusiven Bereichen und Informationen\r

  \r

  ### **Nachteile**\r

  - **Feat-Slot:** Verbraucht einen wertvollen Feat-Slot\r

  - **Spezialisierung:** Fokussiert auf soziale Fähigkeiten\r

  - **Abhängigkeit:** Wirkt nur in urbaner Umgebung\r

  \r

  ---\r

  \r

  ## **Synergien**\r

  \r

  ### **Gute Kombinationen**\r

  - **Diplomat/Anwalt-Beruf:** Verstärkt die diplomatischen Fähigkeiten\r

  - **Händler-Beruf:** Verstärkt die Handelsfähigkeiten\r

  - **Gesellschaftlicher Aufsteiger:** Kombiniert mit sozialem Aufstieg\r

  - **Doppelleben:** Verstärkt die Fähigkeit, Identitäten zu verbergen\r

  \r

  ### **Klassenempfehlungen**\r

  - **Agent:** Spione brauchen soziale Kontakte\r

  - **Diplomat:** Professionelle Diplomaten profitieren am meisten\r

  - **Händler:** Geschäftsleute brauchen Netzwerke\r

  - **Konzernangestellter:** Bürokraten nutzen Netzwerke\r

  \r

  ---\r

  \r

  ## **Taktische Anwendung**\r

  \r

  ### **Soziale Situationen**\r

  - **Verhandlungen:** Bessere Verhandlungsergebnisse\r

  - **Informationen:** Zugang zu wichtigen Informationen\r

  - **Einfluss:** Einfluss über wichtige Personen ausüben\r

  - **Zugang:** Zugang zu exklusiven Bereichen\r

  \r

  ### **Außerhalb des Kampfes**\r

  - **Missionen:** Unterstützung durch Kontakte\r

  - **Geschäfte:** Bessere Geschäftsabschlüsse\r

  - **Politik:** Einfluss auf politische Entscheidungen\r

  - **Gesellschaft:** Bessere gesellschaftliche Position\r

  \r

  ---\r

  \r

  ## **Fluff und Hintergrund**\r

  \r

  ### **Charakterkonzepte**\r

  - **Diplomat:** Professioneller Diplomat mit vielen Kontakten\r

  - **Händler:** Geschäftsmann mit weitreichenden Netzwerken\r

  - **Politiker:** Politiker mit Einfluss und Kontakten\r

  - **Gesellschaftler:** Jemand, der in der High Society verkehrt\r

  \r

  ### **Geschichtliche Integration**\r

  - **Konzerne:** Konzernangestellte und Manager\r

  - **Regierung:** Politiker und Beamte\r

  - **Diplomatie:** Diplomaten und Botschafter\r

  - **Handel:** Händler und Geschäftsleute\r

  \r

  ---\r

  \r

  ## **Regeltechnische Details**\r

  \r

  ### **Interaktionen**\r

  - **Charisma:** Verstärkt das Charisma-Attribut\r

  - **Kontakte:** Erhöht die Anzahl verfügbarer Kontakte\r

  - **Einfluss:** Verstärkt den Einfluss über Kontakte\r

  - **Zugang:** Erhöht den Zugang zu exklusiven Bereichen\r

  \r

  ### **Spezialfälle**\r

  - **Urbane Umgebung:** Funktioniert nur in Städten\r

  - **Ländliche Umgebung:** Keine Boni in ländlichen Gebieten\r

  - **Kontakte:** Erhöht die Anzahl verfügbarer Kontakte\r

  - **Einfluss:** Verstärkt den Einfluss über Kontakte\r

  \r

  ---\r

  \r

  ## **Beschreibung im Spiel**\r

  \r

  Netzwerker ist eine der wichtigsten sozialen Fähigkeiten im Zeitalter des Goldes. In einer Welt voller Hierarchien, Konzerne und politischer Intrigen ist die Fähigkeit, soziale Kontakte zu nutzen, oft der Schlüssel zum Erfolg.\r

  \r

  Diese Fähigkeit repräsentiert nicht nur soziale Kompetenz, sondern auch die Fähigkeit, Beziehungen aufzubauen und zu nutzen. Charaktere mit Netzwerker haben gelernt, ihre sozialen Kontakte optimal zu nutzen und auch in komplexen sozialen Situationen zu navigieren."
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Netzwerker ist eine Fähigkeit, die es Charakteren ermöglicht, ihre sozialen Verbindungen und Kontakte optimal zu nutzen.
traits:
  - trait.general
type: feat
---

# Netzwerker

**Quelle:** Welt-Regelwerk (Zeitalter des Goldes)

---

## **Flavortext**

> „Es ist nicht was du weißt, sondern wen du kennst."

Netzwerker ist eine Fähigkeit, die es Charakteren ermöglicht, ihre sozialen Verbindungen und Kontakte optimal zu nutzen. In der urbanen Welt des Zeitalters des Goldes, wo Beziehungen und Netzwerke oft wichtiger sind als individuelle Fähigkeiten, ist diese Fähigkeit von unschätzbarem Wert.

---

## **Übersicht**

Netzwerker verbessert die Fähigkeit eines Charakters, in urbaner Umgebung zu navigieren und seine sozialen Kontakte zu nutzen. Diese Fähigkeit ist besonders nützlich für Diplomaten, Händler und alle, die auf soziale Interaktionen angewiesen sind.

---

## **Mechanische Details**

### **Voraussetzungen**
- **Stufe:** 1+
- **Charisma:** 13+

### **Effekt**
- **Gesellschaft:** +2 auf Gesellschaftswürfe in urbaner Umgebung
- **Diplomatie:** +2 auf Diplomatiewürfe in urbaner Umgebung
- **Stapelbar:** Nein (nur einmal erhältlich)

### **Anwendungsbereiche**
- **Netzwerken:** Neue Kontakte knüpfen
- **Informationen:** Informationen über Kontakte sammeln
- **Einfluss:** Einfluss über Kontakte ausüben
- **Zugang:** Zugang zu exklusiven Bereichen erhalten

---

## **Spielmechanische Auswirkungen**

### **Vorteile**
- **Soziale Kontrolle:** Bessere Kontrolle über soziale Situationen
- **Informationen:** Zugang zu wichtigen Informationen
- **Einfluss:** Einfluss über wichtige Personen ausüben
- **Zugang:** Zugang zu exklusiven Bereichen und Informationen

### **Nachteile**
- **Feat-Slot:** Verbraucht einen wertvollen Feat-Slot
- **Spezialisierung:** Fokussiert auf soziale Fähigkeiten
- **Abhängigkeit:** Wirkt nur in urbaner Umgebung

---

## **Synergien**

### **Gute Kombinationen**
- **Diplomat/Anwalt-Beruf:** Verstärkt die diplomatischen Fähigkeiten
- **Händler-Beruf:** Verstärkt die Handelsfähigkeiten
- **Gesellschaftlicher Aufsteiger:** Kombiniert mit sozialem Aufstieg
- **Doppelleben:** Verstärkt die Fähigkeit, Identitäten zu verbergen

### **Klassenempfehlungen**
- **Agent:** Spione brauchen soziale Kontakte
- **Diplomat:** Professionelle Diplomaten profitieren am meisten
- **Händler:** Geschäftsleute brauchen Netzwerke
- **Konzernangestellter:** Bürokraten nutzen Netzwerke

---

## **Taktische Anwendung**

### **Soziale Situationen**
- **Verhandlungen:** Bessere Verhandlungsergebnisse
- **Informationen:** Zugang zu wichtigen Informationen
- **Einfluss:** Einfluss über wichtige Personen ausüben
- **Zugang:** Zugang zu exklusiven Bereichen

### **Außerhalb des Kampfes**
- **Missionen:** Unterstützung durch Kontakte
- **Geschäfte:** Bessere Geschäftsabschlüsse
- **Politik:** Einfluss auf politische Entscheidungen
- **Gesellschaft:** Bessere gesellschaftliche Position

---

## **Fluff und Hintergrund**

### **Charakterkonzepte**
- **Diplomat:** Professioneller Diplomat mit vielen Kontakten
- **Händler:** Geschäftsmann mit weitreichenden Netzwerken
- **Politiker:** Politiker mit Einfluss und Kontakten
- **Gesellschaftler:** Jemand, der in der High Society verkehrt

### **Geschichtliche Integration**
- **Konzerne:** Konzernangestellte und Manager
- **Regierung:** Politiker und Beamte
- **Diplomatie:** Diplomaten und Botschafter
- **Handel:** Händler und Geschäftsleute

---

## **Regeltechnische Details**

### **Interaktionen**
- **Charisma:** Verstärkt das Charisma-Attribut
- **Kontakte:** Erhöht die Anzahl verfügbarer Kontakte
- **Einfluss:** Verstärkt den Einfluss über Kontakte
- **Zugang:** Erhöht den Zugang zu exklusiven Bereichen

### **Spezialfälle**
- **Urbane Umgebung:** Funktioniert nur in Städten
- **Ländliche Umgebung:** Keine Boni in ländlichen Gebieten
- **Kontakte:** Erhöht die Anzahl verfügbarer Kontakte
- **Einfluss:** Verstärkt den Einfluss über Kontakte

---

## **Beschreibung im Spiel**

Netzwerker ist eine der wichtigsten sozialen Fähigkeiten im Zeitalter des Goldes. In einer Welt voller Hierarchien, Konzerne und politischer Intrigen ist die Fähigkeit, soziale Kontakte zu nutzen, oft der Schlüssel zum Erfolg.

Diese Fähigkeit repräsentiert nicht nur soziale Kompetenz, sondern auch die Fähigkeit, Beziehungen aufzubauen und zu nutzen. Charaktere mit Netzwerker haben gelernt, ihre sozialen Kontakte optimal zu nutzen und auch in komplexen sozialen Situationen zu navigieren.
