---
category: general
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: +1 Packen/Stoßen
examples: []
id: feat.general.athletischer-kampfstil
legacy:
  notes: []
  paths:
    - feats/feats_overview.md
    - feats/feat_athletischer_kampfstil.md
level: 1
name: Athletischer Kampfstil
prerequisites:
  - all:
      - attribute:
          gte: 15
          id: strength
references: []
rulesText: "# Athletischer Kampfstil\r

  \r

  **Quelle:** Welt-Regelwerk (Zeitalter des Goldes)\r

  \r

  ---\r

  \r

  ## **Flavortext**\r

  \r

  > „Kraft ohne Technik ist nutzlos, aber Technik ohne Kraft ist ebenfalls nutzlos.\"\r

  \r

  Athletischer Kampfstil kombiniert körperliche Stärke mit Kampftechnik, um Gegner durch physische Manipulation zu überwältigen. Diese Fähigkeit ist besonders nützlich für Kämpfer, die ihre körperliche Überlegenheit im Nahkampf ausnutzen wollen.\r

  \r

  ---\r

  \r

  ## **Übersicht**\r

  \r

  Athletischer Kampfstil verbessert die Fähigkeit eines Charakters, Gegner zu packen, zu stoßen oder anderweitig physisch zu manipulieren. Diese Fähigkeit ist besonders wertvoll für Kämpfer, die ihre körperliche Stärke optimal einsetzen wollen.\r

  \r

  ---\r

  \r

  ## **Mechanische Details**\r

  \r

  ### **Voraussetzungen**\r

  - **Stufe:** 1+\r

  - **Stärke:** 15+\r

  \r

  ### **Effekt**\r

  - **Packen/Stoßen:** +1 auf Angriffswürfe beim Packen oder Stoßen\r

  - **Stapelbar:** Nein (nur einmal erhältlich)\r

  \r

  ### **Anwendungsbereiche**\r

  - **Packen:** Gegner festhalten und kontrollieren\r

  - **Stoßen:** Gegner zurückdrängen oder zu Boden werfen\r

  - **Wurftechniken:** Gegner werfen oder umwerfen\r

  - **Körperliche Manipulation:** Gegner in Position bringen\r

  \r

  ---\r

  \r

  ## **Spielmechanische Auswirkungen**\r

  \r

  ### **Vorteile**\r

  - **Kampfkontrolle:** Gegner physisch kontrollieren\r

  - **Taktische Optionen:** Mehr Möglichkeiten im Kampf\r

  - **Schwächere Gegner:** Besonders effektiv gegen schwächere Gegner\r

  - **Umgebung:** Nutzung der Umgebung für taktische Vorteile\r

  \r

  ### **Nachteile**\r

  - **Feat-Slot:** Verbraucht einen wertvollen Feat-Slot\r

  - **Spezialisierung:** Fokussiert auf physische Kampftechniken\r

  - **Abhängigkeit:** Wirkt nur bei Packen und Stoßen\r

  \r

  ---\r

  \r

  ## **Synergien**\r

  \r

  ### **Gute Kombinationen**\r

  - **Zähigkeit:** Überleben während des Nahkampfs\r

  - **Defensiver Kämpfer:** Kombiniert Offensive mit Verteidigung\r

  - **Taktiker:** Unterstützt Verbündete durch Kontrolle\r

  - **Athletik-Fertigkeit:** Verstärkt die Grundfertigkeit\r

  \r

  ### **Klassenempfehlungen**\r

  - **Söldner:** Professionelle Kämpfer profitieren am meisten\r

  - **Wächter:** Defensive Kämpfer können Gegner kontrollieren\r

  - **Raufbold:** Straßenkämpfer nutzen physische Techniken\r

  - **Agent:** Spione können Gegner neutralisieren\r

  \r

  ---\r

  \r

  ## **Taktische Anwendung**\r

  \r

  ### **Kampf**\r

  - **Kontrolle:** Gegner festhalten und handlungsunfähig machen\r

  - **Positionierung:** Gegner in ungünstige Positionen bringen\r

  - **Schutz:** Verbündete vor Angriffen schützen\r

  - **Umgebung:** Gegner gegen Wände oder Hindernisse drängen\r

  \r

  ### **Außerhalb des Kampfes**\r

  - **Verhaftung:** Verdächtige festhalten\r

  - **Schutz:** Andere vor Gefahren schützen\r

  - **Transport:** Widerständige Personen transportieren\r

  - **Einschüchterung:** Physische Präsenz demonstrieren\r

  \r

  ---\r

  \r

  ## **Fluff und Hintergrund**\r

  \r

  ### **Charakterkonzepte**\r

  - **Kampfsportler:** Professioneller Ringer oder Boxer\r

  - **Bodyguard:** Leibwächter mit physischen Techniken\r

  - **Polizist:** Ordnungshüter mit Verhaftungstechniken\r

  - **Söldner:** Professioneller Kämpfer mit Nahkampferfahrung\r

  \r

  ### **Geschichtliche Integration**\r

  - **Polizei:** Verhaftungstechniken und Kontrolle\r

  - **Konzerne:** Sicherheitskräfte und Bodyguards\r

  - **Militär:** Nahkampfausbildung und Kontrolle\r

  - **Untergrund:** Straßenkämpfer und Schläger\r

  \r

  ---\r

  \r

  ## **Regeltechnische Details**\r

  \r

  ### **Interaktionen**\r

  - **Athletik-Fertigkeit:** Verstärkt die Grundfertigkeit\r

  - **Stärke-Attribut:** Verstärkt den Effekt zusätzlich\r

  - **Größe:** Funktioniert besser gegen kleinere Gegner\r

  - **Rüstung:** Gegner in Rüstung sind schwerer zu kontrollieren\r

  \r

  ### **Spezialfälle**\r

  - **Größenunterschiede:** Bonus gegen kleinere Gegner\r

  - **Rüstung:** Mali gegen schwere Rüstung\r

  - **Magie:** Magische Effekte können die Kontrolle beeinträchtigen\r

  - **Technologie:** Technologische Hilfsmittel können helfen\r

  \r

  ---\r

  \r

  ## **Beschreibung im Spiel**\r

  \r

  Athletischer Kampfstil ist eine der wichtigsten Fähigkeiten für Nahkämpfer im Zeitalter des Goldes. In einer Welt voller technologischer Waffen und magischer Effekte ist die Fähigkeit, Gegner physisch zu kontrollieren, oft der Schlüssel zum Sieg.\r

  \r

  Diese Fähigkeit repräsentiert nicht nur körperliche Stärke, sondern auch die Technik und das Wissen, wie man diese Stärke optimal einsetzt. Charaktere mit athletischem Kampfstil haben gelernt, ihre körperliche Überlegenheit zu nutzen und Gegner durch physische Manipulation zu überwältigen."
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Athletischer Kampfstil kombiniert körperliche Stärke mit Kampftechnik, um Gegner durch physische Manipulation zu überwältigen.
traits:
  - trait.general
type: feat
---

# Athletischer Kampfstil

**Quelle:** Welt-Regelwerk (Zeitalter des Goldes)

---

## **Flavortext**

> „Kraft ohne Technik ist nutzlos, aber Technik ohne Kraft ist ebenfalls nutzlos."

Athletischer Kampfstil kombiniert körperliche Stärke mit Kampftechnik, um Gegner durch physische Manipulation zu überwältigen. Diese Fähigkeit ist besonders nützlich für Kämpfer, die ihre körperliche Überlegenheit im Nahkampf ausnutzen wollen.

---

## **Übersicht**

Athletischer Kampfstil verbessert die Fähigkeit eines Charakters, Gegner zu packen, zu stoßen oder anderweitig physisch zu manipulieren. Diese Fähigkeit ist besonders wertvoll für Kämpfer, die ihre körperliche Stärke optimal einsetzen wollen.

---

## **Mechanische Details**

### **Voraussetzungen**
- **Stufe:** 1+
- **Stärke:** 15+

### **Effekt**
- **Packen/Stoßen:** +1 auf Angriffswürfe beim Packen oder Stoßen
- **Stapelbar:** Nein (nur einmal erhältlich)

### **Anwendungsbereiche**
- **Packen:** Gegner festhalten und kontrollieren
- **Stoßen:** Gegner zurückdrängen oder zu Boden werfen
- **Wurftechniken:** Gegner werfen oder umwerfen
- **Körperliche Manipulation:** Gegner in Position bringen

---

## **Spielmechanische Auswirkungen**

### **Vorteile**
- **Kampfkontrolle:** Gegner physisch kontrollieren
- **Taktische Optionen:** Mehr Möglichkeiten im Kampf
- **Schwächere Gegner:** Besonders effektiv gegen schwächere Gegner
- **Umgebung:** Nutzung der Umgebung für taktische Vorteile

### **Nachteile**
- **Feat-Slot:** Verbraucht einen wertvollen Feat-Slot
- **Spezialisierung:** Fokussiert auf physische Kampftechniken
- **Abhängigkeit:** Wirkt nur bei Packen und Stoßen

---

## **Synergien**

### **Gute Kombinationen**
- **Zähigkeit:** Überleben während des Nahkampfs
- **Defensiver Kämpfer:** Kombiniert Offensive mit Verteidigung
- **Taktiker:** Unterstützt Verbündete durch Kontrolle
- **Athletik-Fertigkeit:** Verstärkt die Grundfertigkeit

### **Klassenempfehlungen**
- **Söldner:** Professionelle Kämpfer profitieren am meisten
- **Wächter:** Defensive Kämpfer können Gegner kontrollieren
- **Raufbold:** Straßenkämpfer nutzen physische Techniken
- **Agent:** Spione können Gegner neutralisieren

---

## **Taktische Anwendung**

### **Kampf**
- **Kontrolle:** Gegner festhalten und handlungsunfähig machen
- **Positionierung:** Gegner in ungünstige Positionen bringen
- **Schutz:** Verbündete vor Angriffen schützen
- **Umgebung:** Gegner gegen Wände oder Hindernisse drängen

### **Außerhalb des Kampfes**
- **Verhaftung:** Verdächtige festhalten
- **Schutz:** Andere vor Gefahren schützen
- **Transport:** Widerständige Personen transportieren
- **Einschüchterung:** Physische Präsenz demonstrieren

---

## **Fluff und Hintergrund**

### **Charakterkonzepte**
- **Kampfsportler:** Professioneller Ringer oder Boxer
- **Bodyguard:** Leibwächter mit physischen Techniken
- **Polizist:** Ordnungshüter mit Verhaftungstechniken
- **Söldner:** Professioneller Kämpfer mit Nahkampferfahrung

### **Geschichtliche Integration**
- **Polizei:** Verhaftungstechniken und Kontrolle
- **Konzerne:** Sicherheitskräfte und Bodyguards
- **Militär:** Nahkampfausbildung und Kontrolle
- **Untergrund:** Straßenkämpfer und Schläger

---

## **Regeltechnische Details**

### **Interaktionen**
- **Athletik-Fertigkeit:** Verstärkt die Grundfertigkeit
- **Stärke-Attribut:** Verstärkt den Effekt zusätzlich
- **Größe:** Funktioniert besser gegen kleinere Gegner
- **Rüstung:** Gegner in Rüstung sind schwerer zu kontrollieren

### **Spezialfälle**
- **Größenunterschiede:** Bonus gegen kleinere Gegner
- **Rüstung:** Mali gegen schwere Rüstung
- **Magie:** Magische Effekte können die Kontrolle beeinträchtigen
- **Technologie:** Technologische Hilfsmittel können helfen

---

## **Beschreibung im Spiel**

Athletischer Kampfstil ist eine der wichtigsten Fähigkeiten für Nahkämpfer im Zeitalter des Goldes. In einer Welt voller technologischer Waffen und magischer Effekte ist die Fähigkeit, Gegner physisch zu kontrollieren, oft der Schlüssel zum Sieg.

Diese Fähigkeit repräsentiert nicht nur körperliche Stärke, sondern auch die Technik und das Wissen, wie man diese Stärke optimal einsetzt. Charaktere mit athletischem Kampfstil haben gelernt, ihre körperliche Überlegenheit zu nutzen und Gegner durch physische Manipulation zu überwältigen.
