---
category: skill
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: Schwarzmärkte, +1 Täuschung
examples: []
id: feat.general.schattendealer
legacy:
  notes: []
  paths:
    - feats/feats_overview.md
    - feats/feat_schattendealer.md
level: 1
name: Schattendealer
prerequisites:
  - all:
      - attribute:
          gte: 13
          id: charisma
references: []
rulesText: "# Schattendealer\r

  \r

  **Quelle:** Welt-Regelwerk (Zeitalter des Goldes)\r

  \r

  ---\r

  \r

  ## **Flavortext**\r

  \r

  > „Im Schatten der Stadt gibt es andere Regeln, andere Preise und andere Wahrheiten.\"\r

  \r

  Schattendealer ist eine Fähigkeit, die es Charakteren ermöglicht, in der Unterwelt und auf Schwarzmärkten zu operieren. In der urbanen Welt des Zeitalters des Goldes, wo illegale Geschäfte und versteckte Märkte alltäglich sind, ist diese Fähigkeit von unschätzbarem Wert.\r

  \r

  ---\r

  \r

  ## **Übersicht**\r

  \r

  Schattendealer verbessert die Fähigkeit eines Charakters, in illegalen Transaktionen und auf Schwarzmärkten zu operieren. Diese Fähigkeit ist besonders nützlich für Kriminelle, Schmuggler und alle, die auf versteckte Geschäfte angewiesen sind.\r

  \r

  ---\r

  \r

  ## **Mechanische Details**\r

  \r

  ### **Voraussetzungen**\r

  - **Stufe:** 1+\r

  - **Charisma:** 13+\r

  \r

  ### **Effekt**\r

  - **Schwarzmärkte:** Zugang zu Schwarzmärkten\r

  - **Täuschung:** +1 auf Täuschungswürfe in illegalen Transaktionen\r

  - **Diplomatie:** +1 auf Diplomatiewürfe in illegalen Transaktionen\r

  - **Stapelbar:** Nein (nur einmal erhältlich)\r

  \r

  ### **Anwendungsbereiche**\r

  - **Schwarzmärkte:** Zugang zu illegalen Waren\r

  - **Schmuggel:** Illegale Waren transportieren\r

  - **Erpressung:** Erpressung und Drohungen\r

  - **Korruption:** Korruption und Bestechung\r

  \r

  ---\r

  \r

  ## **Spielmechanische Auswirkungen**\r

  \r

  ### **Vorteile**\r

  - **Zugang:** Zugang zu illegalen Waren und Dienstleistungen\r

  - **Preise:** Bessere Preise auf Schwarzmärkten\r

  - **Kontakte:** Kontakte in der Unterwelt\r

  - **Informationen:** Zugang zu geheimen Informationen\r

  \r

  ### **Nachteile**\r

  - **Feat-Slot:** Verbraucht einen wertvollen Feat-Slot\r

  - **Spezialisierung:** Fokussiert auf illegale Aktivitäten\r

  - **Risiko:** Erhöhtes Risiko bei Entdeckung\r

  - **Reputation:** Kann die Reputation beeinträchtigen\r

  \r

  ---\r

  \r

  ## **Synergien**\r

  \r

  ### **Gute Kombinationen**\r

  - **Krimineller-Beruf:** Verstärkt die kriminellen Fähigkeiten\r

  - **Schmuggler:** Verstärkt die Schmuggelfähigkeiten\r

  - **Erpresser:** Verstärkt die Erpressungsfähigkeiten\r

  - **Doppelleben:** Verstärkt die Fähigkeit, Identitäten zu verbergen\r

  \r

  ### **Klassenempfehlungen**\r

  - **Agent:** Spione brauchen Zugang zu Schwarzmärkten\r

  - **Krimineller:** Professionelle Kriminelle profitieren am meisten\r

  - **Schmuggler:** Schmuggler brauchen Zugang zu illegalen Waren\r

  - **Erpresser:** Erpresser brauchen Zugang zu geheimen Informationen\r

  \r

  ---\r

  \r

  ## **Taktische Anwendung**\r

  \r

  ### **Illegale Aktivitäten**\r

  - **Schmuggel:** Illegale Waren transportieren\r

  - **Erpressung:** Erpressung und Drohungen\r

  - **Korruption:** Korruption und Bestechung\r

  - **Informationen:** Zugang zu geheimen Informationen\r

  \r

  ### **Außerhalb des Kampfes**\r

  - **Missionen:** Unterstützung durch Unterweltkontakte\r

  - **Geschäfte:** Bessere Geschäfte auf Schwarzmärkten\r

  - **Schutz:** Schutz durch Unterweltorganisationen\r

  - **Informationen:** Zugang zu geheimen Informationen\r

  \r

  ---\r

  \r

  ## **Fluff und Hintergrund**\r

  \r

  ### **Charakterkonzepte**\r

  - **Schmuggler:** Professioneller Schmuggler mit Kontakten\r

  - **Erpresser:** Erpresser mit Zugang zu geheimen Informationen\r

  - **Krimineller:** Krimineller mit Unterweltkontakten\r

  - **Schattenmann:** Jemand, der im Schatten operiert\r

  \r

  ### **Geschichtliche Integration**\r

  - **Untergrund:** Kriminelle Organisationen und Gangs\r

  - **Schwarzmärkte:** Illegale Waren und Dienstleistungen\r

  - **Korruption:** Korrupte Beamte und Konzernangestellte\r

  - **Schmuggel:** Schmugglerringe und kriminelle Netzwerke\r

  \r

  ---\r

  \r

  ## **Regeltechnische Details**\r

  \r

  ### **Interaktionen**\r

  - **Charisma:** Verstärkt das Charisma-Attribut\r

  - **Kontakte:** Erhöht die Anzahl verfügbarer Unterweltkontakte\r

  - **Zugang:** Erhöht den Zugang zu Schwarzmärkten\r

  - **Preise:** Verbessert die Preise auf Schwarzmärkten\r

  \r

  ### **Spezialfälle**\r

  - **Schwarzmärkte:** Funktioniert nur auf Schwarzmärkten\r

  - **Legale Geschäfte:** Keine Boni bei legalen Geschäften\r

  - **Entdeckung:** Erhöhtes Risiko bei Entdeckung\r

  - **Reputation:** Kann die Reputation beeinträchtigen\r

  \r

  ---\r

  \r

  ## **Beschreibung im Spiel**\r

  \r

  Schattendealer ist eine der wichtigsten Fähigkeiten für Charaktere, die in der Unterwelt operieren. In einer Welt voller Hierarchien, Konzerne und politischer Intrigen ist die Fähigkeit, auf Schwarzmärkten zu operieren, oft der Schlüssel zum Erfolg.\r

  \r

  Diese Fähigkeit repräsentiert nicht nur kriminelle Kompetenz, sondern auch die Fähigkeit, in der Unterwelt zu navigieren und versteckte Geschäfte zu tätigen. Charaktere mit Schattendealer haben gelernt, ihre Unterweltkontakte optimal zu nutzen und auch in komplexen kriminellen Situationen zu navigieren."
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Schattendealer ist eine Fähigkeit, die es Charakteren ermöglicht, in der Unterwelt und auf Schwarzmärkten zu operieren.
traits:
  - trait.general
type: feat
---

# Schattendealer

**Quelle:** Welt-Regelwerk (Zeitalter des Goldes)

---

## **Flavortext**

> „Im Schatten der Stadt gibt es andere Regeln, andere Preise und andere Wahrheiten."

Schattendealer ist eine Fähigkeit, die es Charakteren ermöglicht, in der Unterwelt und auf Schwarzmärkten zu operieren. In der urbanen Welt des Zeitalters des Goldes, wo illegale Geschäfte und versteckte Märkte alltäglich sind, ist diese Fähigkeit von unschätzbarem Wert.

---

## **Übersicht**

Schattendealer verbessert die Fähigkeit eines Charakters, in illegalen Transaktionen und auf Schwarzmärkten zu operieren. Diese Fähigkeit ist besonders nützlich für Kriminelle, Schmuggler und alle, die auf versteckte Geschäfte angewiesen sind.

---

## **Mechanische Details**

### **Voraussetzungen**
- **Stufe:** 1+
- **Charisma:** 13+

### **Effekt**
- **Schwarzmärkte:** Zugang zu Schwarzmärkten
- **Täuschung:** +1 auf Täuschungswürfe in illegalen Transaktionen
- **Diplomatie:** +1 auf Diplomatiewürfe in illegalen Transaktionen
- **Stapelbar:** Nein (nur einmal erhältlich)

### **Anwendungsbereiche**
- **Schwarzmärkte:** Zugang zu illegalen Waren
- **Schmuggel:** Illegale Waren transportieren
- **Erpressung:** Erpressung und Drohungen
- **Korruption:** Korruption und Bestechung

---

## **Spielmechanische Auswirkungen**

### **Vorteile**
- **Zugang:** Zugang zu illegalen Waren und Dienstleistungen
- **Preise:** Bessere Preise auf Schwarzmärkten
- **Kontakte:** Kontakte in der Unterwelt
- **Informationen:** Zugang zu geheimen Informationen

### **Nachteile**
- **Feat-Slot:** Verbraucht einen wertvollen Feat-Slot
- **Spezialisierung:** Fokussiert auf illegale Aktivitäten
- **Risiko:** Erhöhtes Risiko bei Entdeckung
- **Reputation:** Kann die Reputation beeinträchtigen

---

## **Synergien**

### **Gute Kombinationen**
- **Krimineller-Beruf:** Verstärkt die kriminellen Fähigkeiten
- **Schmuggler:** Verstärkt die Schmuggelfähigkeiten
- **Erpresser:** Verstärkt die Erpressungsfähigkeiten
- **Doppelleben:** Verstärkt die Fähigkeit, Identitäten zu verbergen

### **Klassenempfehlungen**
- **Agent:** Spione brauchen Zugang zu Schwarzmärkten
- **Krimineller:** Professionelle Kriminelle profitieren am meisten
- **Schmuggler:** Schmuggler brauchen Zugang zu illegalen Waren
- **Erpresser:** Erpresser brauchen Zugang zu geheimen Informationen

---

## **Taktische Anwendung**

### **Illegale Aktivitäten**
- **Schmuggel:** Illegale Waren transportieren
- **Erpressung:** Erpressung und Drohungen
- **Korruption:** Korruption und Bestechung
- **Informationen:** Zugang zu geheimen Informationen

### **Außerhalb des Kampfes**
- **Missionen:** Unterstützung durch Unterweltkontakte
- **Geschäfte:** Bessere Geschäfte auf Schwarzmärkten
- **Schutz:** Schutz durch Unterweltorganisationen
- **Informationen:** Zugang zu geheimen Informationen

---

## **Fluff und Hintergrund**

### **Charakterkonzepte**
- **Schmuggler:** Professioneller Schmuggler mit Kontakten
- **Erpresser:** Erpresser mit Zugang zu geheimen Informationen
- **Krimineller:** Krimineller mit Unterweltkontakten
- **Schattenmann:** Jemand, der im Schatten operiert

### **Geschichtliche Integration**
- **Untergrund:** Kriminelle Organisationen und Gangs
- **Schwarzmärkte:** Illegale Waren und Dienstleistungen
- **Korruption:** Korrupte Beamte und Konzernangestellte
- **Schmuggel:** Schmugglerringe und kriminelle Netzwerke

---

## **Regeltechnische Details**

### **Interaktionen**
- **Charisma:** Verstärkt das Charisma-Attribut
- **Kontakte:** Erhöht die Anzahl verfügbarer Unterweltkontakte
- **Zugang:** Erhöht den Zugang zu Schwarzmärkten
- **Preise:** Verbessert die Preise auf Schwarzmärkten

### **Spezialfälle**
- **Schwarzmärkte:** Funktioniert nur auf Schwarzmärkten
- **Legale Geschäfte:** Keine Boni bei legalen Geschäften
- **Entdeckung:** Erhöhtes Risiko bei Entdeckung
- **Reputation:** Kann die Reputation beeinträchtigen

---

## **Beschreibung im Spiel**

Schattendealer ist eine der wichtigsten Fähigkeiten für Charaktere, die in der Unterwelt operieren. In einer Welt voller Hierarchien, Konzerne und politischer Intrigen ist die Fähigkeit, auf Schwarzmärkten zu operieren, oft der Schlüssel zum Erfolg.

Diese Fähigkeit repräsentiert nicht nur kriminelle Kompetenz, sondern auch die Fähigkeit, in der Unterwelt zu navigieren und versteckte Geschäfte zu tätigen. Charaktere mit Schattendealer haben gelernt, ihre Unterweltkontakte optimal zu nutzen und auch in komplexen kriminellen Situationen zu navigieren.
