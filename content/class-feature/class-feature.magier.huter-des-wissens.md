---
choiceIds: []
classId: class.magier
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: "Dein Name ist in den geheimen Archiven der Welt vermerkt. Du erhältst: - Resistenz 5 gegen magischen Schaden, - Immunität gegen geistige Kontrolle, - und die Fähigkeit, Dispel Magic einmal pro Stunde kostenlos zu wirken. ---"
examples: []
id: class-feature.magier.huter-des-wissens
legacy:
  notes: []
  paths:
    - classes/klasse_magier.md
level: 20
name: Hüter des Wissens
prerequisites: []
references: []
rulesText: "Dein Name ist in den geheimen Archiven der Welt vermerkt.  \r

  Du erhältst:  \r

  - Resistenz 5 gegen magischen Schaden,  \r

  - Immunität gegen geistige Kontrolle,  \r

  - und die Fähigkeit, *Dispel Magic* einmal pro Stunde kostenlos zu wirken.\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Dein Name ist in den geheimen Archiven der Welt vermerkt.
traits:
  - trait.legacy
type: class-feature
---

Dein Name ist in den geheimen Archiven der Welt vermerkt.  
Du erhältst:  
- Resistenz 5 gegen magischen Schaden,  
- Immunität gegen geistige Kontrolle,  
- und die Fähigkeit, *Dispel Magic* einmal pro Stunde kostenlos zu wirken.

---
