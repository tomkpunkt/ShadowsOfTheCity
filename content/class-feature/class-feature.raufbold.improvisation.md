---
choiceIds: []
classId: class.raufbold
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: Du kannst aus allem eine Waffe machen. Wenn du mit einem improvisierten Gegenstand (z. B. Stuhl, Rohr, Flasche) angreifst, erhältst du +1 auf Angriffswürfe und verursachst zusätzlichen Schaden in Höhe deines STÄ-Modifikators. Du kannst außerdem jede Runde eine Aktion verwenden, um eine Umgebungssituation zu deinem Vorteil zu nutzen (SG 15; z. B. Licht, Geräusch, Deckung). ---
examples: []
id: class-feature.raufbold.improvisation
legacy:
  notes: []
  paths:
    - classes/klasse_raufbold.md
level: 1
name: Improvisation
prerequisites: []
references: []
rulesText: "Du kannst aus allem eine Waffe machen.  \r

  Wenn du mit einem improvisierten Gegenstand (z. B. Stuhl, Rohr, Flasche) angreifst, erhältst du +1 auf Angriffswürfe und verursachst zusätzlichen Schaden in Höhe deines STÄ-Modifikators.  \r

  Du kannst außerdem jede Runde eine Aktion verwenden, um eine Umgebungssituation zu deinem Vorteil zu nutzen (SG 15; z. B. Licht, Geräusch, Deckung).\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du kannst aus allem eine Waffe machen.
traits:
  - trait.legacy
type: class-feature
---

Du kannst aus allem eine Waffe machen.  
Wenn du mit einem improvisierten Gegenstand (z. B. Stuhl, Rohr, Flasche) angreifst, erhältst du +1 auf Angriffswürfe und verursachst zusätzlichen Schaden in Höhe deines STÄ-Modifikators.  
Du kannst außerdem jede Runde eine Aktion verwenden, um eine Umgebungssituation zu deinem Vorteil zu nutzen (SG 15; z. B. Licht, Geräusch, Deckung).

---
