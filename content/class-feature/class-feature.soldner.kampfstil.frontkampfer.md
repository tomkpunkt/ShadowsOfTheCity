---
choiceIds: []
classId: class.soldner
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: Du stürzt dich in den Nahkampf und hältst die Linie. - Erhalte den Feat Power Attack. - Wenn du eine Rüstung trägst, erhältst du +1 AC gegen Nahkampfangriffe.
examples: []
id: class-feature.soldner.kampfstil.frontkampfer
legacy:
  notes: []
  paths:
    - classes/klasse_soeldner.md
level: 1
name: Frontkämpfer
prerequisites: []
references: []
rulesText: "Du stürzt dich in den Nahkampf und hältst die Linie.  \r

  - Erhalte den Feat *Power Attack*.  \r

  - Wenn du eine Rüstung trägst, erhältst du +1 AC gegen Nahkampfangriffe."
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du stürzt dich in den Nahkampf und hältst die Linie.
traits:
  - trait.class-option
  - trait.class-option.soldner.kampfstil
type: class-feature
---

Du stürzt dich in den Nahkampf und hältst die Linie.  
- Erhalte den Feat *Power Attack*.  
- Wenn du eine Rüstung trägst, erhältst du +1 AC gegen Nahkampfangriffe.
