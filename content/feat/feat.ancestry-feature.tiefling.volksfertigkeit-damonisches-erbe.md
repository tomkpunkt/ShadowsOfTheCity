---
ancestryId: ancestry.tiefling
category: ancestry
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: "Die fremde Energie in deinem Blut schützt dich. Effekt: Du erhältst Resistenz 2 gegen Feuer und Elektrizität. Wenn du Schaden durch magische Energie erleidest, reduziere ihn um 1 (Pfeil oder Zauber). ---"
examples: []
id: feat.ancestry-feature.tiefling.volksfertigkeit-damonisches-erbe
legacy:
  notes: []
  paths:
    - races/tieflinge_zeitalter_des_goldes.md
level: 1
name: Volksfertigkeit – Dämonisches Erbe
prerequisites:
  - ancestry:
      id: ancestry.tiefling
references: []
rulesText: "Die fremde Energie in deinem Blut schützt dich.  \r

  **Effekt:** Du erhältst Resistenz 2 gegen Feuer und Elektrizität. Wenn du Schaden durch magische Energie erleidest, reduziere ihn um 1 (Pfeil oder Zauber).\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Die fremde Energie in deinem Blut schützt dich.
traits:
  - trait.ancestry
type: feat
---

Die fremde Energie in deinem Blut schützt dich.  
**Effekt:** Du erhältst Resistenz 2 gegen Feuer und Elektrizität. Wenn du Schaden durch magische Energie erleidest, reduziere ihn um 1 (Pfeil oder Zauber).

---
