---
choiceIds: []
classId: class.okkultist
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: "Du bist kein Sterblicher mehr, sondern ein Knotenpunkt des Geflechts. Du erhältst: - Immunität gegen mentale und Furchteffekte, - Resistenz 5 gegen magischen Schaden, - und kannst einmal pro Tag Wish oder ein äquivalentes Ritual ausführen – doch jedes Mal steigt das Risiko, dauerhaft 1 Attributspunkt zu verlieren. ---"
examples: []
id: class-feature.okkultist.verbotener-aufstieg
legacy:
  notes: []
  paths:
    - classes/klasse_okkultist.md
level: 20
name: Verbotener Aufstieg
prerequisites: []
references: []
rulesText: "Du bist kein Sterblicher mehr, sondern ein Knotenpunkt des Geflechts.  \r

  Du erhältst:  \r

  - Immunität gegen mentale und Furchteffekte,  \r

  - Resistenz 5 gegen magischen Schaden,  \r

  - und kannst einmal pro Tag *Wish* oder ein äquivalentes Ritual ausführen – doch jedes Mal steigt das Risiko, dauerhaft 1 Attributspunkt zu verlieren.\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du bist kein Sterblicher mehr, sondern ein Knotenpunkt des Geflechts.
traits:
  - trait.legacy
type: class-feature
---

Du bist kein Sterblicher mehr, sondern ein Knotenpunkt des Geflechts.  
Du erhältst:  
- Immunität gegen mentale und Furchteffekte,  
- Resistenz 5 gegen magischen Schaden,  
- und kannst einmal pro Tag *Wish* oder ein äquivalentes Ritual ausführen – doch jedes Mal steigt das Risiko, dauerhaft 1 Attributspunkt zu verlieren.

---
