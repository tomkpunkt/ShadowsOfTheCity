---
choiceIds: []
classId: class.mediziner
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: Du bist Wissenschaftler der Heilkunst. - Du erhältst Fertigkeitstraining in Wissenschaft und Naturkunde. - Du kannst einmal pro Tag ein Heilserum herstellen (Heilung 1W8 oder +2 auf Zähigkeit für 1 Stunde). - Du erhältst +1 auf Würfe gegen Krankheiten und Gifte.
examples: []
id: class-feature.mediziner.fachrichtung.forscher
legacy:
  notes: []
  paths:
    - classes/klasse_mediziner.md
level: 1
name: Forscher
prerequisites: []
references: []
rulesText: "Du bist Wissenschaftler der Heilkunst.  \r

  - Du erhältst Fertigkeitstraining in Wissenschaft und Naturkunde.  \r

  - Du kannst einmal pro Tag ein Heilserum herstellen (Heilung 1W8 oder +2 auf Zähigkeit für 1 Stunde).  \r

  - Du erhältst +1 auf Würfe gegen Krankheiten und Gifte."
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du bist Wissenschaftler der Heilkunst.
traits:
  - trait.class-option
  - trait.class-option.mediziner.fachrichtung
type: class-feature
---

Du bist Wissenschaftler der Heilkunst.  
- Du erhältst Fertigkeitstraining in Wissenschaft und Naturkunde.  
- Du kannst einmal pro Tag ein Heilserum herstellen (Heilung 1W8 oder +2 auf Zähigkeit für 1 Stunde).  
- Du erhältst +1 auf Würfe gegen Krankheiten und Gifte.
