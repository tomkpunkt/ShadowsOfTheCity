---
choiceIds: []
classId: class.raufbold
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: Du erkennst Schwächen und Gelegenheiten. Wenn du in einer Runde Schaden erleidest, erhältst du im nächsten Zug +1 auf Angriff und Schaden gegen denselben Gegner. Wenn du einen Gegner zu Boden bringst, erhältst du +2 auf Einschüchtern für 1 Minute. ---
examples: []
id: class-feature.raufbold.kampfinstinkt
legacy:
  notes: []
  paths:
    - classes/klasse_raufbold.md
level: 3
name: Kampfinstinkt
prerequisites: []
references: []
rulesText: "Du erkennst Schwächen und Gelegenheiten.  \r

  Wenn du in einer Runde Schaden erleidest, erhältst du im nächsten Zug +1 auf Angriff und Schaden gegen denselben Gegner.  \r

  Wenn du einen Gegner zu Boden bringst, erhältst du +2 auf Einschüchtern für 1 Minute.\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du erkennst Schwächen und Gelegenheiten.
traits:
  - trait.legacy
type: class-feature
---

Du erkennst Schwächen und Gelegenheiten.  
Wenn du in einer Runde Schaden erleidest, erhältst du im nächsten Zug +1 auf Angriff und Schaden gegen denselben Gegner.  
Wenn du einen Gegner zu Boden bringst, erhältst du +2 auf Einschüchtern für 1 Minute.

---
