---
ancestryId: ancestry.elf
category: ancestry
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: Du kannst feine emotionale und magische Schwingungen wahrnehmen. Du erhältst den Zauber Detect Magic als arkane Fähigkeit, die du unbegrenzt oft einsetzen kannst.
examples: []
id: feat.ancestry.elf.aurenleser
legacy:
  notes: []
  paths:
    - races/elfen_zeitalter_des_goldes.md
level: 13
name: Aurenleser
prerequisites:
  - ancestry:
      id: ancestry.elf
references: []
rulesText: Du kannst feine emotionale und magische Schwingungen wahrnehmen. Du erhältst den Zauber *Detect Magic* als arkane Fähigkeit, die du unbegrenzt oft einsetzen kannst.
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du kannst feine emotionale und magische Schwingungen wahrnehmen.
traits:
  - trait.ancestry
type: feat
---

Du kannst feine emotionale und magische Schwingungen wahrnehmen. Du erhältst den Zauber *Detect Magic* als arkane Fähigkeit, die du unbegrenzt oft einsetzen kannst.
