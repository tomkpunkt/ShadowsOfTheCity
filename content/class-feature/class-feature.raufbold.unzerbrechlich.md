---
choiceIds: []
classId: class.raufbold
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: Du bist das Sinnbild des Überlebens. Wenn du auf 0 TP fallen würdest, bleibst du bei 1 TP und stehst weiter. Du erhältst außerdem Resistenz 3 gegen physischen Schaden und +1 auf Willenswürfe gegen Furcht. ---
examples: []
id: class-feature.raufbold.unzerbrechlich
legacy:
  notes: []
  paths:
    - classes/klasse_raufbold.md
level: 15
name: Unzerbrechlich
prerequisites: []
references: []
rulesText: "Du bist das Sinnbild des Überlebens.  \r

  Wenn du auf 0 TP fallen würdest, bleibst du bei 1 TP und stehst weiter.  \r

  Du erhältst außerdem Resistenz 3 gegen physischen Schaden und +1 auf Willenswürfe gegen Furcht.\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du bist das Sinnbild des Überlebens.
traits:
  - trait.legacy
type: class-feature
---

Du bist das Sinnbild des Überlebens.  
Wenn du auf 0 TP fallen würdest, bleibst du bei 1 TP und stehst weiter.  
Du erhältst außerdem Resistenz 3 gegen physischen Schaden und +1 auf Willenswürfe gegen Furcht.

---
