---
choiceIds: []
classId: class.ingenieur
editorialStatus: reviewed
effects:
  - classification: partially-structured
    kind: text
    machineReadable: false
    text: Du erkennst sofort, wie Dinge funktionieren – oder wie man sie zerstört. Du erhältst +2 auf Wahrnehmung und Handwerk, wenn du Geräte, Waffen oder Gebäude untersuchst. Einmal pro Stunde kannst du die Schwachstelle eines mechanischen Gegners oder Fahrzeugs aufdecken (+2 auf Angriffe dagegen für 1 Minute). ---
examples: []
id: class-feature.ingenieur.konstrukteursblick
legacy:
  notes: []
  paths:
    - classes/klasse_ingenieur.md
level: 11
name: Konstrukteursblick
prerequisites: []
references: []
rulesText: "Du erkennst sofort, wie Dinge funktionieren – oder wie man sie zerstört.  \r

  Du erhältst +2 auf Wahrnehmung und Handwerk, wenn du Geräte, Waffen oder Gebäude untersuchst.  \r

  Einmal pro Stunde kannst du die Schwachstelle eines mechanischen Gegners oder Fahrzeugs aufdecken (+2 auf Angriffe dagegen für 1 Minute).\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Du erkennst sofort, wie Dinge funktionieren – oder wie man sie zerstört.
traits:
  - trait.legacy
type: class-feature
---

Du erkennst sofort, wie Dinge funktionieren – oder wie man sie zerstört.  
Du erhältst +2 auf Wahrnehmung und Handwerk, wenn du Geräte, Waffen oder Gebäude untersuchst.  
Einmal pro Stunde kannst du die Schwachstelle eines mechanischen Gegners oder Fahrzeugs aufdecken (+2 auf Angriffe dagegen für 1 Minute).

---
