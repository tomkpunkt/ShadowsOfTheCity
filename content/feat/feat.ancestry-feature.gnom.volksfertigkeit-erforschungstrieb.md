---
ancestryId: ancestry.gnom
category: ancestry
editorialStatus: reviewed
effects:
  - kind: text
    machineReadable: false
    text: "Dein unstillbarer Drang, Neues zu entdecken, treibt dich an. Effekt: Du erhältst einen +1 Situationsbonus auf Würfe für Wissen (Wahrnehmung, Arkane Kunde, Handwerk), wenn du etwas Neues untersuchst oder eine unbekannte Technologie analysierst. ---"
examples: []
id: feat.ancestry-feature.gnom.volksfertigkeit-erforschungstrieb
legacy:
  notes: []
  paths:
    - races/gnome_zeitalter_des_goldes.md
level: 1
name: Volksfertigkeit – Erforschungstrieb
prerequisites:
  - ancestry:
      id: ancestry.gnom
references: []
rulesText: "Dein unstillbarer Drang, Neues zu entdecken, treibt dich an.  \r

  **Effekt:** Du erhältst einen +1 Situationsbonus auf Würfe für Wissen (Wahrnehmung, Arkane Kunde, Handwerk), wenn du etwas Neues untersuchst oder eine unbekannte Technologie analysierst.\r

  \r

  ---"
schemaVersion: 1
source: legacy.world-rules
status: legacy
summary: Dein unstillbarer Drang, Neues zu entdecken, treibt dich an.
traits:
  - trait.ancestry
type: feat
---

Dein unstillbarer Drang, Neues zu entdecken, treibt dich an.  
**Effekt:** Du erhältst einen +1 Situationsbonus auf Würfe für Wissen (Wahrnehmung, Arkane Kunde, Handwerk), wenn du etwas Neues untersuchst oder eine unbekannte Technologie analysierst.

---
